...
PlayerFrameMorph.prototype.read = function (file) {
	...
	//try {
		...
		stage.penCanvas = newCanvas(stage.extent());
		if (this.info.at('penTrails')) {
			var ctx = stage.penCanvas.getContext('2d');
			ctx.drawImage(this.info.at('penTrails').image, 0, 0);
		}
		this.setStage(stage);
	//} catch (e) {
	//	alert(e);
...
ImageMedia.prototype.initBeforeLoad = function  () {
	...
	if (this.form.base64)
		this.base64 = this.compositeForm ? this.compositeForm.base64 : this.form.base64;
	if (!this.base64)
		this.image = null;
	else
		this.image = newImage(this.base64);
